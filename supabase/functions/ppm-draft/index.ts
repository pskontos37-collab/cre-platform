...
const SECTIONS: Record<string, { coverage: string; exemplar: string }> = {
  exec_summary: {
    coverage: `Paragraph 1: the transaction opener. For a JV ACQUISITION, begin "This transaction presents selected accredited investors with an opportunity to invest in <investorCompanyName>, a new Delaware limited liability company (the "Wilkow Investor Company"), which will enter into a joint venture..." naming the JV partner and the property (include the JV-history note if provided). For a PREFERRED-EQUITY RECAP (dealStructure = pref_equity_recap), instead begin "This transaction presents selected accredited investors with an opportunity to invest in <investorCompanyName>, a newly formed Delaware limited liability company (the "Wilkow Investor Company"), which will enter into a recapitalization agreement with the current owner of the Property, <existingOwnerName> (the "Existing Owner"), by providing up to <prefEquityAmount> in preferred equity capital..." and describe the existing mortgage (existingLoanBalance/Rate/Maturity) and that the MJW Member becomes managing member with an MJW affiliate as property manager. Paragraph 2: the partner/owner credibility paragraph (use jvPartnerBlurb for a JV; for a recap, summarize the recap rationale). Paragraph 3: property overview — size, location, occupancy, anchor/tenant mix, location strengths. Paragraph 4 (headed "Wilkow Investor Company"): the investment case + the raise (units, unit price) and the preference waterfall in prose (Class A preference for a JV; the MJW Member preferred-return priorities from recapTiers for a recap). Paragraph 5 (headed "Projected Yields and Assumptions"): the base-case forecast sentence with hold/exit and the projected IRR, average cash flow yield, and equity multiple (for a recap these are the MJW Member's preferred returns); add the Upside Case paragraph only if upside data is present.`,
...
- DEAL STRUCTURE: if the data sheet's dealStructure is "pref_equity_recap", this is NOT a purchase/JV acquisition. It is a PREFERRED-EQUITY RECAPITALIZATION: the Wilkow Investor Company (the "MJW Member") contributes preferred equity into the existing owner's ("<existingOwnerName>") capital stack; there is no purchase price (use estimatedPropertyValue and the existing capital stack), no new JV partner, and the return is the MJW Member's PREFERRED return per the recap waterfall (recapTiers), not a JV promote. Speak of "the recapitalization" and "the Investment," never "the acquisition" or "the Joint Venture." Use prefEquityAmount, existingLoanBalance/Rate/Maturity, refinancingAssumptions, and recapTiers where relevant.`
...
- Preferred-equity recap (only if the text describes a recapitalization, not a purchase): existingOwnerName, estimatedPropertyValue, estValueNote, existingLoanOriginal, existingLoanBalance, existingLoanRate (decimal), existingLoanMaturity, currentOwnershipEquity, prefEquityAmount, prefEquityUse, refinancingAssumptions, recapSaleKicker, recapTiers [{priority, recipient, hurdle}]
