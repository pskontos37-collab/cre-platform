...
serve(async (req) => {
  ...
  try {
    ...
    if (!row.abstract) throw new Error('No abstract on this agreement — run agreement-abstract first')
...
${briefParts.length ? `\nSOURCE DOCUMENTS (structured briefs):\n${briefParts.join('\n\n')}` : ''}`
    ...
    let qa: any
    try {
      qa = await anthropicJson(anthropicKey, MODEL, [...attachments, { type: 'text', text: prompt }], 12000)
    } catch (e) {
      if (!attachments.length || !isCapError(e)) throw e
      qa = await anthropicJson(anthropicKey, MODEL, [{ type: 'text', text: prompt }], 12000)
      attachments = []
    }

    const qaStatus = deriveStatus(qa)
    const { error: upErr } = await sb.from(table).update({
      qa, qa_status: qaStatus, qa_model: MODEL, qa_at: new Date().toISOString(),
