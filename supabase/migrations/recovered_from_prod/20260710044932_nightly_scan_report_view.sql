...
-- Nightly scan digest as a single ready-to-email row. Computed on read (no cron,
-- no workstation dependency). Power Automate GETs this once/night and sends body_html.
-- Window = last 24h. security_invoker so caller RLS applies; service key sees all.
...
from agg cross join nf_by_prop cross join nf_list;

grant select on public.v_scan_report to authenticated, service_role;
comment on view public.v_scan_report is 'Nightly scan digest (last 24h) as one row incl. body_html; Power Automate GETs + emails it.';
