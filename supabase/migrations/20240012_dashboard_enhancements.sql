...
create table public.pct_rent_records (
  id                        uuid        primary key default uuid_generate_v4(),
  lease_id                  uuid        not null references leases(id) on delete cascade,
  property_id               uuid        not null references properties(id) on delete cascade,
  tenant_id                 uuid        not null references tenants(id) on delete cascade,
...
-- ── Indexes ────────────────────────────────────────────────────
create index idx_cam_recon_property_id  on public.cam_reconciliations(property_id);
create index idx_cam_recon_status       on public.cam_reconciliations(status);
create index idx_cam_recon_year         on public.cam_reconciliations(period_year);
create index idx_lease_pay_property_id  on public.lease_payments(property_id);
create index idx_lease_pay_due_date     on public.lease_payments(due_date);
create index idx_lease_pay_paid_date    on public.lease_payments(paid_date);
create index idx_pct_rent_property_id   on public.pct_rent_records(property_id);
create index idx_pct_rent_lease_id      on public.pct_rent_records(lease_id);
create index idx_pct_rent_year          on public.pct_rent_records(period_year);
