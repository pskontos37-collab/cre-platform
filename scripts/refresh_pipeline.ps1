...
#   5. extract_underwriting.ps1 fill still-blank return metrics from deal docs
...
Step "1/5 Sync weekly pipeline book"    { & (Join-Path $here 'load_acq_pipeline.ps1') }
Step "2/5 Link deal folders"            { & (Join-Path $here 'link_deal_folders.ps1') }
Step "3/5 Mirror new documents"         { & (Join-Path $here 'mirror_deal_docs.ps1') -Apply }
Step "4/5 Site plans"                   { & (Join-Path $here 'extract_site_plans.ps1') -Apply }
Step "5/5 Underwriting auto-fill"       { & (Join-Path $here 'extract_underwriting.ps1') -Apply }
